...
class AchatIllimixScreen extends StatefulWidget {
  final String myNumber;
  final String token;

  const AchatIllimixScreen({
    super.key,
  ...
  @override
  State<AchatIllimixScreen> createState() => _AchatIllimixScreenState();
}

class _AchatIllimixScreenState extends State<AchatIllimixScreen> {
  ...
  Future<void> _loadPasses() async {
    ...
    try {
      final passes = await ApiService.getPassIllimix(widget.token);
      setState(() {
        _allPasses = passes.map((p) => {
          ...
          "minutesAppels": p["minutesAppels"] as int,
          "volumeDonneeMo": p["volumeDonneeMo"] as int,
          "nbMessages": p["nbMessages"] as int,
        }).toList();
      });
    } catch (e) {
      setState(() {
  ...
  void _goToConfirmation() async {
    ...
    if (_selectedPass == null) {
      setState(() {
        _errorMessage = "Veuillez sélectionner un pass illimix dans la liste.";
      });
      return;
    }
  ...
  void _confirmPurchase(StateSetter setDialogState, String paymentSource) async {
    ...
    try {
      await ApiService.purchasePassIllimix(
      ...
      children: [
        InkWell(
          ...
          child: Padding(
            ...
            child: Row(
              children: [
                const Icon(Icons.all_inclusive_rounded, color: orangeColor, size: 24),
            ...
            child: categoryPasses.isEmpty
                ...
                : Column(
                    children: categoryPasses.map((pass) {
                      ...
                      final minutes = pass["minutesAppels"] as int;
                      final minText = minutes >= 60
                          ? "${(minutes / 60).toStringAsFixed(0)}h"
                          : "$minutes min";
                            ...
                            children: [
                              ...
                              Row(
                                ...
                                children: [
                                  ...
                                  if (isSelected)
                                    const Icon(Icons.check_circle, color: orangeColor, size: 20),
                                ],
                              ),
                              ...
                              // Allocation row matching the user's specific request
                              Row(
                                children: [
                                  // Minutes
                                  const Icon(Icons.phone_rounded, color: Colors.greenAccent, size: 15),
                                  ...
                                  // SMS
                                  const Icon(Icons.message_rounded, color: Colors.orangeAccent, size: 15),
                                  const SizedBox(width: 4),
                                  Text(
                                    "${pass["nbMessages"]} SMS",
                                    style: const TextStyle(color: Colors.white70, fontSize: 12, fontWeight: FontWeight.bold),
                                  ...
                                  // Data
                                  const Icon(Icons.language_rounded, color: Colors.blueAccent, size: 15),
                                  const SizedBox(width: 4),
                                  Text(
                                    volumeText,
                                    style: const TextStyle(color: Colors.white70, fontSize: 12, fontWeight: FontWeight.bold),
            ...
            : Form(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    // Bottom buy/cancel options matching layout style
                    Padding(
                      padding: const EdgeInsets.all(16.0),
                      child: Row(
                        children: [
